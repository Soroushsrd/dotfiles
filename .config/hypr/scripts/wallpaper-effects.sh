...
if [ $1 == "reload" ]; then
    ...
    ~/.config/hypr/scripts/wallpaper.sh $(cat $cache_file)
else
    # Open rofi to select the wallpaper effect
    options="$(ls ~/.config/hypr/effects/wallpaper/)\noff"
    # Open rofi
    choice=$(echo -e "$options" | rofi -dmenu -replace -config ~/.config/rofi/config-themes.rasi -i -no-show-icons -l 5 -width 30 -p "Wallpaper Effect")
    if [ ! -z $choice ]; then
